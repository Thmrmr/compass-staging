...
return(<div style={{...CS,border:"1px solid rgba(0,135,159,0.15)",boxShadow:"0 2px 12px rgba(0,135,159,0.06)"}}><div style={{height:2,background:"linear-gradient(90deg,#D0F94A,#00D49C,#00879F)"}}/><div style={{padding:"10px 16px",display:"flex",alignItems:"center",gap:8}}>
...
<div style={{height:320,overflowY:"auto",padding:14}}>{ms.length===0&&<div style={{textAlign:"center",paddingTop:50,color:"var(--muted)",fontSize:13}}>Ask about deals, pipeline, or strategy</div>}{ms.map((m,i)=><div key={i} style={{marginBottom:10,display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}><div style={{maxWidth:"80%"}}><div style={{padding:"9px 13px",borderRadius:11,fontSize:13,lineHeight:1.6,background:m.role==="user"?"#0D1B1E":"rgba(0,135,159,0.06)",color:m.role==="user"?"#fff":"var(--text)",border:m.role==="assistant"?"1px solid rgba(0,135,159,0.1)":"none",whiteSpace:"normal"}}>{m.role==="assistant"?<MD text={m.content}/>:m.content}</div>{m.role==="assistant"&&<button onClick={()=>speakTTS(m.content,elKey)} style={{marginTop:3,background:"none",border:"none",cursor:"pointer",color:"var(--muted)",padding:2}}><Volume2 size={12}/></button>}</div></div>)}{b&&<div style={{...M,color:"var(--muted)",fontSize:11}}>Thinking...</div>}<div ref={end}/></div>
...
return(<div><div style={{marginBottom:20}}><div style={{...T,fontSize:22,fontWeight:800}}>Marketing</div><div style={{fontSize:13,color:"var(--muted)",marginTop:2}}>Campaigns, leads, content assets, and calendar.</div></div>
  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
    <KPI v={leads.length} l="ACTIVE LEADS" c="#00879F"/>
    <KPI v={camps.filter(x=>x.status==="Active").length} l="ACTIVE CAMPAIGNS" c="#00D49C"/>
    <KPI v={assets.length} l="CONTENT ASSETS" c="#5A7A00"/>
...
<div style={{...CS,padding:"16px 20px",marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center",background:"rgba(0,212,156,0.02)",border:"1px solid rgba(0,212,156,0.1)"}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
    <div style={{...CS,padding:20}}>
      ...
      {camps.slice(0,5).map(cp=>{const cpLeads=leads.filter(l=>l.source_campaign_id===cp.id).length;const cpDeals=leads.filter(l=>l.source_campaign_id===cp.id&&l.converted_deal_id).length;return(
        <div key={cp.id} style={{padding:"8px 0",borderBottom:"1px solid var(--border)"}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{...M,fontSize:10,color:"var(--muted)"}}>{cp.type} · {cp.status}</span><span style={{...M,fontSize:10,color:"#00D49C"}}>{cpLeads} leads → {cpDeals} deals</span></div>
        </div>);})}
    ...
    <div style={{...CS,padding:20}}>
      ...
      {(()=>{const funnelData=[{label:"Leads",count:leads.length,color:"#8A9BAA"},{label:"Warm",count:leads.filter(l=>l.status==="Warm").length,color:"#FFB800"},{label:"Hot",count:leads.filter(l=>l.status==="Hot").length,color:"#FF6B6B"},{label:"Ready",count:leads.filter(l=>l.status==="Ready").length,color:"#00D49C"},{label:"Converted",count:leads.filter(l=>l.status==="Converted").length,color:"#00879F"},{label:"Won",count:leads.filter(l=>l.converted_deal_id).length,color:"#D0F94A"}];const maxCount=Math.max(...funnelData.map(f=>f.count),1);return(
        <div style={{display:"flex",alignItems:"flex-end",gap:8,height:120}}>
          {funnelData.map(f=><div key={f.label} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            ...
            <div style={{width:"100%",background:f.color,borderRadius:4,height:Math.max(f.count/maxCount*80,4)}}/>
...
function AgentsTab({token,deals,leads,assets,debriefs,onRefresh,claudeKey,onOpenDeal}){
  ...
  return(<div>
    ...
    <div style={{...CS,padding:20,marginBottom:20,border:"1px solid rgba(0,135,159,0.1)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}><Globe size={14} color="#00879F"/><span style={{...M,fontSize:10,letterSpacing:"0.1em",color:"#00879F"}}>LIVING SECTOR INTELLIGENCE</span></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
        {SECTORS.map(s=>{const sDeals=deals.filter(d=>d.sector===s&&d.status==="Active");const sVal=sDeals.reduce((x,d)=>x+(d.expected_value||0),0);const avgScore=sDeals.length?Math.round(sDeals.reduce((x,d)=>x+(d.deal_score||0),0)/sDeals.length):0;return(
          <div key={s} style={{padding:"12px 14px",background:"var(--panel2)",borderRadius:8,borderLeft:"3px solid "+(XC[s]||"#8A9BAA")}}>
            ...
            <div style={{...M,fontSize:9,color:avgScore>=60?"#00D49C":avgScore>=30?"#FFB800":"var(--muted)",marginTop:4}}>Avg score: {avgScore||"—"}</div>
          </div>);})}
      </div>
    </div>
...
return(<div>
  ...
  {newUser&&<div style={{...CS,padding:20,marginBottom:20}}>
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><UserPlus size={16} color="#00879F"/><span style={{...T,fontSize:16,fontWeight:700}}>Create New User</span></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
      <div><label style={{...LB,color:"#00879F"}}>FULL NAME</label><input placeholder="Your full name" style={IP} onChange={e=>sNU(p=>({...p,name:e.target.value}))}/></div>
      <div><label style={{...LB,color:"#00879F"}}>EMAIL ADDRESS</label><input placeholder="name@humain.com" style={IP} onChange={e=>sNU(p=>({...p,email:e.target.value}))}/></div>
      <div><label style={{...LB,color:"#00879F"}}>TEMPORARY PASSWORD</label><input placeholder="Minimum 8 characters" type="password" style={IP} onChange={e=>sNU(p=>({...p,pass:e.target.value}))}/></div>
      <div><label style={{...LB,color:"#00879F"}}>ACCESS ROLE</label><select style={IP} onChange={e=>sNU(p=>({...p,role:e.target.value}))}><option value="member">Member</option><option value="manager">Manager</option><option value="admin">Admin</option><option value="viewer">Viewer</option></select></div>
      <div><label style={{...LB,color:"#00879F"}}>DEPARTMENT</label><select style={IP} onChange={e=>sNU(p=>({...p,dept:e.target.value}))}><option value="">Select</option><option>BD</option><option>Marketing</option><option>Executive</option><option>Engineering</option><option>Operations</option></select></div>
      <div><label style={{...LB,color:"#00879F"}}>PRIMARY SECTOR FOCUS</label><select style={IP} onChange={e=>sNU(p=>({...p,sector:e.target.value}))}><option value="">All sectors</option>{SECTORS.map(s=><option key={s}>{s}</option>)}</select></div>
    </div>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280,1fr))",gap:12,marginBottom:24}}>
    {users.map(u=><div key={u.id} style={{...CS,padding:"16px 18px"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
        <div style={{width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,#00879F,#00D49C)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:"#fff",flexShrink:0}}>{(u.full_name||u.email||"?")[0].toUpperCase()}</div>
        ...
        {u.id===profile?.id&&<span style={{...M,fontSize:8,color:"#5A7A00",padding:"2px 6px",background:"rgba(90,122,0,0.08)",borderRadius:3}}>YOU</span>}
      </div>
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        <select value={u.role||"member"} onChange={async(e)=>{const nr=e.target.value;try{await q("/rest/v1/profiles?id=eq."+u.id,token,{method:"PATCH",body:JSON.stringify({role:nr})});sU(p=>p.map(x=>x.id===u.id?{...x,role:nr}:x));}catch(ex){alert(ex.message);}}} style={{...M,fontSize:10,padding:"3px 8px",borderRadius:4,background:u.role==="admin"?"rgba(208,249,74,0.1)":"rgba(0,135,159,0.06)",color:u.role==="admin"?"#6B8C00":"#00879F",border:"none",cursor:"pointer"}}><option value="admin">Admin</option><option value="manager">Manager</option><option value="member">Member</option><option value="viewer">Viewer</option></select>
        <select value={u.team||""} onChange={async(e)=>{try{await q("/rest/v1/profiles?id=eq."+u.id,token,{method:"PATCH",body:JSON.stringify({team:e.target.value})});sU(p=>p.map(x=>x.id===u.id?{...x,team:e.target.value}:x));}catch(ex){}}} style={{...M,fontSize:10,padding:"3px 8px",borderRadius:4,border:"1px solid var(--border)",background:"transparent",color:"var(--sub)",cursor:"pointer"}}><option value="">Team</option><option>Executive</option><option>BD</option><option>Marketing</option><option>Engineering</option></select>
...
return(<div>
  ...
  {!chatActive&&<div style={{display:"flex",alignItems:"center",gap:8,margin:"16px 0 12px"}}><div style={{width:6,height:6,borderRadius:3,background:"#00D49C"}}/><span style={{...M,fontSize:10,color:"var(--sub)"}}>{AGENTS.length} agents active</span><span style={{...M,fontSize:10,color:"#FFB800"}}>{pendingActs} actions pending</span></div>}
  {!chatActive&&<div style={{display:"flex",gap:6,marginBottom:16}}>{[{id:"foryou",label:"For you"},{id:"pipeline",label:"Pipeline"},{id:"intelligence",label:"Intelligence"},{id:"marketing",label:"Marketing"}].map(cat=><button key={cat.id} onClick={()=>sHomeCat(cat.id)} style={{...M,fontSize:10,padding:"6px 14px",borderRadius:6,border:"1px solid var(--border)",background:homeCat===cat.id?"rgba(0,135,159,0.06)":"transparent",color:homeCat===cat.id?"#00879F":"var(--muted)",cursor:"pointer"}}>{cat.label}</button>)}</div>}
    ...
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {suggestions.slice(0,4).map(s=>{const prioColor=s.priority==="critical"?"#FF4B4B":s.priority==="high"?"#FFB800":"#00879F";return(
        <div key={s.id} style={{display:"flex",alignItems:"flex-start",gap:12,padding:"12px 16px",background:"var(--card-bg)",border:"1px solid var(--border)",borderRadius:10,cursor:"pointer"}}
          ...
          <div style={{...M,fontSize:9,color:prioColor,flexShrink:0,padding:"2px 6px",borderRadius:3,background:prioColor+"10"}}>{s.agent_name}</div>
        </div>);})}
    </div>
  </div>}
  {!chatActive&&<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
    <KPI v={ac.length} l="ACTIVE DEALS" c="#00879F"/>
    <KPI v={"SAR "+(pv/1e6).toFixed(1)+"M"} l="PIPELINE" c="#00D49C"/>
    <KPI v={pendingActs} l="AGENT ACTIONS" c="#FFB800"/>
    <KPI v={readyLeads} l="READY LEADS" c="#5A7A00"/>
  </div>}
...
return(<div>
  ...
  {isA&&<div style={{...CS,padding:20,marginBottom:20,border:"1px solid rgba(90,122,0,0.15)"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><div style={{display:"flex",alignItems:"center",gap:8}}><Shield size={14} color="#5A7A00"/><span style={{...M,fontSize:10,letterSpacing:"0.1em",color:"#5A7A00"}}>EXECUTIVE SUMMARY</span></div></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:10,marginBottom:16}}>
      <div style={{padding:10,background:"var(--panel2)",borderRadius:8,textAlign:"center"}}><div style={{...T,fontSize:18,fontWeight:800,color:"#00879F"}}>SAR {(pv/1e6).toFixed(1)}M</div><div style={{...M,fontSize:8,color:"var(--muted)",marginTop:2}}>PIPELINE</div></div>
      <div style={{padding:10,background:"var(--panel2)",borderRadius:8,textAlign:"center"}}><div style={{...T,fontSize:18,fontWeight:800,color:"#00D49C"}}>SAR {(wv/1e6).toFixed(1)}M</div><div style={{...M,fontSize:8,color:"var(--muted)",marginTop:2}}>WEIGHTED</div></div>
      <div style={{padding:10,background:"var(--panel2)",borderRadius:8,textAlign:"center"}}><div style={{...T,fontSize:18,fontWeight:800,color:"#5A7A00"}}>SAR {(wonV/1e6).toFixed(1)}M</div><div style={{...M,fontSize:8,color:"var(--muted)",marginTop:2}}>WON</div></div>
      ...
      <div style={{padding:10,background:"var(--panel2)",borderRadius:8,textAlign:"center"}}><div style={{...T,fontSize:18,fontWeight:800,color:pv>=10000000?"#00D49C":"#FFB800"}}>{wv>=5000000?"HEALTHY":"AT RISK"}</div><div style={{...M,fontSize:8,color:"var(--muted)",marginTop:2}}>PIPELINE HEALTH</div></div>
    </div>
  </div>}
  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
    <div style={{...CS,padding:20}}>
      ...
      {STAGES.map(s=>{const cnt=ac.filter(d=>d.stage===s).length;const val=ac.filter(d=>d.stage===s).reduce((x,d)=>x+(d.expected_value||0),0);const pct=ac.length?Math.round(cnt/ac.length*100):0;return(
        <div key={s} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
          ...
          <div style={{flex:1,height:24,background:"var(--panel2)",borderRadius:4,overflow:"hidden",position:"relative"}}>
            <div style={{width:pct+"%",height:"100%",background:SC[s],borderRadius:4,display:"flex",alignItems:"center",paddingLeft:6}}>
    ...
    <div style={{...CS,padding:20}}>
      ...
      {SECTORS.map(s=>{const cnt=deals.filter(d=>d.sector===s).length;const pct=deals.length?Math.round(cnt/deals.length*100):0;return(
        <div key={s} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
          <div style={{width:10,height:10,borderRadius:5,background:XC[s],flexShrink:0}}/>
          <span style={{fontSize:12,flex:1}}>{s}</span>
          <div style={{width:60,height:6,background:"var(--panel2)",borderRadius:3,overflow:"hidden"}}><div style={{width:pct+"%",height:"100%",background:XC[s],borderRadius:3}}/></div>
    ...
    <div style={{...CS,padding:20}}>
      ...
      {SECTORS.map(s=>{const total=deals.filter(d=>d.sector===s).length;const won2=deals.filter(d=>d.sector===s&&d.status==="Won").length;const rate=total?Math.round(won2/total*100):0;return(
        <div key={s} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
          ...
          <div style={{flex:1,height:6,background:"var(--panel2)",borderRadius:3,overflow:"hidden"}}><div style={{width:rate+"%",height:"100%",background:XC[s],borderRadius:3}}/></div>
    ...
    <div style={{...CS,padding:20}}>
      ...
      {Object.entries(prods).sort((a,b)=>b[1].value-a[1].value).slice(0,8).map(([name,data])=>(
        <div key={name} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
          ...
          <span style={{...M,fontSize:10,color:"#00879F"}}>{data.count} deal{data.count>1?"s":""}</span>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
    <div style={{...CS,padding:16,textAlign:"center"}}><div style={{...T,fontSize:22,fontWeight:800,color:"#00879F"}}>{ac.length}</div><div style={{...M,fontSize:9,color:"var(--muted)"}}>ACTIVE</div></div>
    <div style={{...CS,padding:16,textAlign:"center"}}><div style={{...T,fontSize:22,fontWeight:800,color:"#00D49C"}}>{wo.length}</div><div style={{...M,fontSize:9,color:"var(--muted)"}}>WON</div></div>
    ...
    {(()=>{const months=[];const now=new Date();for(let i=5;i>=0;i--){const d=new Date(now.getFullYear(),now.getMonth()-i,1);months.push({label:d.toLocaleDateString("en-GB",{month:"short"}),count:deals.filter(dl=>{const cd=new Date(dl.created_at||dl.updated_at);return cd.getMonth()===d.getMonth()&&cd.getFullYear()===d.getFullYear();}).length});}const maxM=Math.max(...months.map(m=>m.count),1);return(
      <div style={{display:"flex",alignItems:"flex-end",gap:10,height:80}}>
        {months.map(m=><div key={m.label} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
          ...
          <div style={{width:"100%",background:"#00879F",borderRadius:4,height:Math.max(m.count/maxM*60,4)}}/>
  ...
  <div style={{display:"flex",flexDirection:"column",gap:16,marginBottom:32}}>
    <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
      <div style={{width:36,height:36,borderRadius:"50%",background:"#00879F",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",...T,fontSize:16,fontWeight:800,flexShrink:0}}>1</div>
      <div><div style={{...T,fontSize:16,fontWeight:700,marginBottom:4}}>Open Your Supabase Project</div><div style={{fontSize:13,color:"var(--sub)"}}>Go to <a href="https://supabase.com" target="_blank" rel="noreferrer" style={{color:"#00879F",textDecoration:"none"}}>supabase.com</a> and open project <a href="https://nujczhqxcxuppatnjbon.supabase.co" target="_blank" rel="noreferrer" style={{color:"#00879F",textDecoration:"none",...M,fontSize:11}}>nujczhqxcxuppatnjbon.supabase.co</a></div></div>
    </div>
    <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
      <div style={{width:36,height:36,borderRadius:"50%",background:"#00D49C",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",...T,fontSize:16,fontWeight:800,flexShrink:0}}>2</div>
    ...
    <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
      <div style={{width:36,height:36,borderRadius:"50%",background:"#D0F94A",display:"flex",alignItems:"center",justifyContent:"center",color:"#0D1B1E",...T,fontSize:16,fontWeight:800,flexShrink:0}}>3</div>
  ...
  <div style={{...CS,marginBottom:16}}>
    <div style={{padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid var(--border)"}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}><Database size={14} color="#00879F"/><span style={{...T,fontSize:14,fontWeight:700}}>Full SQL Schema</span><span style={{...M,fontSize:9,color:"var(--muted)"}}>Creates all 11 tables</span></div>
  ...
  <div style={{padding:"14px 18px",background:"rgba(0,212,156,0.04)",border:"1px solid rgba(0,212,156,0.12)",borderRadius:10}}>
    <div style={{fontSize:13,color:"#00D49C",fontWeight:600,marginBottom:4}}>You have already signed in successfully.</div>
    <div style={{fontSize:12,color:"var(--sub)"}}>Run this query now to confirm your admin role:</div>
    <div style={{...M,fontSize:11,color:dark?"#a8d8a8":"var(--sub)",background:dark?"#0D1B1E":"var(--panel2)",padding:"10px 14px",borderRadius:6,marginTop:8}}>UPDATE profiles SET role = 'admin' WHERE email = '{profile?.email||"your@email.com"}';</div>
  </div>
</div>);
    ...
    <nav style={{width:sb?230:56,background:"var(--panel)",borderRight:"1px solid rgba(0,135,159,0.06)",display:"flex",flexDirection:"column",transition:"width .28s cubic-bezier(.4,0,.2,1)",position:"fixed",top:0,left:0,bottom:0,zIndex:100,overflow:"hidden",flexShrink:0}}>
        ...
        <div style={{borderTop:"1px solid rgba(0,135,159,0.06)",padding:sb?"8px 12px":"8px",display:"flex",flexDirection:"column",gap:3,flexShrink:0,alignItems:sb?"stretch":"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"2px 0"}}>
            ...
            {sb&&<div style={{marginLeft:8,overflow:"hidden",flex:1}}><div style={{fontSize:11,fontWeight:600,whiteSpace:"nowrap"}}>{nm}</div>{isA&&<div style={{...M,fontSize:7,color:"#5A7A00",letterSpacing:"0.08em"}}>ADMIN</div>}</div>}
          </div>
          {/* Sign out */}
