...
return(<div style={{position:"fixed",inset:0,display:"flex",fontFamily:"'Inter','DM Sans',sans-serif",background:"#F5F6F8"}}>
  ...
  <div style={{width:"50%",padding:16,display:"flex"}}>
    <div style={{flex:1,background:"linear-gradient(145deg, #0D1B1E 0%, #023c47 50%, #0D1B1E 100%)",borderRadius:20,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",position:"relative",overflow:"hidden"}}>
    {/* Subtle glow */}
    <div style={{position:"absolute",top:"50%",left:"50%",width:500,height:500,borderRadius:"50%",background:"radial-gradient(circle, rgba(0,150,136,0.18) 0%, transparent 60%)",transform:"translate(-50%,-50%)",filter:"blur(40px)",pointerEvents:"none"}}/>
    {/* Title area at top */}
    <div style={{position:"relative",zIndex:2,textAlign:"center",padding:"48px 40px 0",flexShrink:0}}>
      ...
      <div style={{display:"inline-block",padding:"4px 12px",background:"rgba(0,184,156,0.15)",borderRadius:999,fontSize:11,fontWeight:600,color:"#00B89C",letterSpacing:"0.1em"}}>BETA</div>
    </div>
    {/* Hero image */}
    <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",zIndex:2,padding:"20px 24px 32px",width:"100%"}}>
      <img src="/compass-hero.jpg" alt="COMPASS" style={{maxWidth:"100%",maxHeight:"100%",objectFit:"contain",borderRadius:10,opacity:0.95}}/>
    </div>
    </div>
  </div>
</div>);}
        ...
        <div key={item.id} style={{padding:"14px 18px",borderBottom:"1px solid var(--border)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:4,marginLeft:10,flexShrink:0}}>
              ...
              {item.deal_id&&<button onClick={()=>onOpenDeal&&onOpenDeal(item.deal_id)} style={{padding:"3px 8px",borderRadius:8,border:"1px solid var(--border)",background:"none",cursor:"pointer",color:"#009688",fontSize:9,...M}}>View Deal</button>}
            </div>
          </div>
        </div>
      ))}</div>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280,1fr))",gap:12,marginBottom:24}}>
    {users.map(u=><div key={u.id} style={{...CS,padding:"16px 18px"}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        <select value={u.role||"member"} onChange={async(e)=>{const nr=e.target.value;try{await q("/rest/v1/profiles?id=eq."+u.id,token,{method:"PATCH",body:JSON.stringify({role:nr})});sU(p=>p.map(x=>x.id===u.id?{...x,role:nr}:x));}catch(ex){alert(ex.message);}}} style={{...M,fontSize:10,padding:"3px 8px",borderRadius:8,background:u.role==="admin"?"rgba(208,249,74,0.1)":"rgba(0,150,136,0.06)",color:u.role==="admin"?"#5B7A0F":"#009688",border:"none",cursor:"pointer"}}><option value="admin">Admin</option><option value="manager">Manager</option><option value="member">Member</option><option value="viewer">Viewer</option></select>
        <select value={u.team||""} onChange={async(e)=>{try{await q("/rest/v1/profiles?id=eq."+u.id,token,{method:"PATCH",body:JSON.stringify({team:e.target.value})});sU(p=>p.map(x=>x.id===u.id?{...x,team:e.target.value}:x));}catch(ex){}}} style={{...M,fontSize:10,padding:"3px 8px",borderRadius:8,border:"1px solid var(--border)",background:"transparent",color:"var(--sub)",cursor:"pointer"}}><option value="">Team</option><option>Executive</option><option>BD</option><option>Marketing</option><option>Engineering</option></select>
  ...
  return(<div style={{position:"fixed",top:0,right:open?0:-400,width:380,bottom:0,background:"var(--panel)",borderLeft:"1px solid var(--border)",zIndex:250,transition:"right .35s cubic-bezier(0.16,1,0.3,1)",display:"flex",flexDirection:"column",boxShadow:open?"-8px 0 32px rgba(0,0,0,0.1)":"none"}}>
    ...
    <div style={{flex:1,overflowY:"auto"}}>
      {requests.filter(r=>r.status==="Pending").map(r=><div key={r.id} style={{padding:"14px 20px",borderBottom:"1px solid var(--border)",display:"flex",gap:12,alignItems:"flex-start"}}>
        ...
        <button onClick={()=>markRead(r.id)} style={{...M,fontSize:9,padding:"3px 8px",borderRadius:8,border:"1px solid var(--border)",background:"none",color:"var(--muted)",cursor:"pointer"}}>Dismiss</button>
      </div>)}
...
return(<div>
  ...
  return(<div style={{...th,fontFamily:"'ABC Repro','Inter','DM Sans',sans-serif",background:"var(--bg)",color:"var(--text)",minHeight:"100vh",display:"flex"}}>
    <nav onMouseEnter={()=>sSb(true)} onMouseLeave={()=>sSb(false)} style={{width:sb?220:60,display:"flex",flexDirection:"column",alignItems:"center",padding:"14px 0",gap:4,transition:"width .2s cubic-bezier(.16,1,.3,1)",position:"fixed",top:0,left:0,bottom:0,zIndex:100,overflow:"hidden",flexShrink:0,background:dark?"var(--panel)":"transparent"}}>
        {/* Logo: H mark when collapsed, full HUMAIN when expanded */}
        <div style={{display:"flex",alignItems:"center",height:42,padding:"0 10px",margin:"0 6px 18px",width:"calc(100% - 12px)",flexShrink:0}}>
          {sb
            ?<svg width="144" height="22" viewBox="0 0 615 94" fill={dark?"#E8F0F0":"#1A1A1A"} xmlns="http://www.w3.org/2000/svg"><path d="M18 36.2H90.4V56.2H18ZM90.5 19.5H18V.2H0V92.3H18V72.9H90.4V92.3H108.4V.2H90.4V19.6Z"/><path d="M204.4 47.9C204.4 64.5 193.8 76.5 179 76.5S153.6 64.6 153.6 47.9V0H135.6V47.9C135.6 74.4 154.1 94 179 94S222.4 74.4 222.4 47.9V0H204.4V47.9Z"/><path d="M295.5 38.7H294.2L265.9 0H249.6V92.1H267.6V27.7H267.9S269.7 33.6 272.9 38L285.6 55.3H303.6L316.3 38C319.5 33.6 321.3 27.7 321.3 27.7H321.6V92.2H339.6V0H323.3L295 38.7Z"/><path d="M392.3 57.1L408.6 16.6H410.4L426.7 57.1H392.2ZM396.2 0L359.1 92.1H378.2L385.6 73.7H433.4L440.8 92.1H460.4L423.4 0H396.2Z"/><path d="M497.4 0H479.4V92.1H497.4V0Z"/><path d="M597 0V63.6L540.9 0H524.6V92.1H542.6V28.6L598.7 92.2H615V0H597Z"/></svg>
            :<svg width="26" height="24" viewBox="0 0 109 93" fill={dark?"#E8F0F0":"#1A1A1A"} xmlns="http://www.w3.org/2000/svg"><path d="M18 36.2H90.4V56.2H18ZM90.5 19.5H18V.2H0V92.3H18V72.9H90.4V92.3H108.4V.2H90.4V19.6Z"/></svg>
          }
        </div>
